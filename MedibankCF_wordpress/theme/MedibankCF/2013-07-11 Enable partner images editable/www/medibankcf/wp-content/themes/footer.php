...
?>
				<div class="op"><?php the_title(); ?></div>
					...
					<!-- First loops prints out sponsor images in ul elment -->
					<?php 
					$imageCounter = 0;
					$loopCounter=0;
					//catch_that_image_count get the image counter from "Our Partners" post content.
					$imageNumber = catch_that_image_count( get_the_content("__('[...]')" ));
					$limit = 6;
					$extraLoopTimes = $imageNumber%$limit;
					if($imageNumber > 0):					
					?>
					<ul>					
						<?php for($i = $imageCounter; $i<$limit;$i++):?>
						<li>
                    	<?php echo catch_that_image( get_the_content("__('[...]')" ),$i);?>
						</li>
						<?php endfor; $counter = $i;?>					
					</ul>
					<?php endif;?>
					<!-- End of first loops -->
					<!-- Extraloops pagenates sponsor images, and wrap them in "scrollable" div -->
					<?php for($j = $loopCounter;$j < $extraLoopTimes;$j++):?>
					<?php $limit = ($limit + 6);?>
					<div class="scrollable">
					<ul>
						<?php for($i = $counter; $i<$limit;$i++):?>
						<li>
                    	<?php echo catch_that_image( get_the_content("__('[...]')" ),$i);?>
						</li>
						<?php endfor;?>
